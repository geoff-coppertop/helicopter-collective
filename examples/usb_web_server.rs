...
fn encode_mdns_name(buf: &mut [u8], hostname: &str, domain: &str) -> usize {
    let mut pos = 0;
...
/// Returns the number of bytes written.
fn build_mdns_announcement(resp: &mut [u8], name_enc: &[u8], device_ip: [u8; 4], ttl: u32) -> usize {
...
fn names_equal_prefix(query: &[u8], encoded: &[u8]) -> bool {
